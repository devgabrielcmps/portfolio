...
from cotacao import cotacao,cotacao_bitcoin
...
def bitcoin():
   #Criando a janela 
    ...
    #Texto 
    text=ctk.CTkLabel(window,text='BITCOIN',font=fonte)
    #Coloca o texto na tela
    text.grid(padx=120,pady=15)

    #Texto 
    text=ctk.CTkLabel(window,text=f'COTAÇÃO: R$ {float(cotacao_bitcoin()):.3f}',font=fonte)
    #Coloca o texto na tela
    text.grid()
    ...
    def converter():
        ...
        qnt_bitcoin=conversaoInversa.get()
        resultado_texto=""

        try:
          if qnt_real :
            qnt_real=float(qnt_real)
            montante_bitcoin=qnt_real/cotacao_bitcoin()
            resultado_texto=f'R$ {qnt_real:.2f} -> {montante_bitcoin:.2f}  ₿'
          elif qnt_bitcoin:
            qnt_bitcoin=float(qnt_bitcoin)
            montante_real=cotacao_bitcoin()*qnt_bitcoin
            resultado_texto=f'{qnt_bitcoin:.2f} ₿ -> R$ {montante_real:.2f}'
          else:
    ...
    def mostrar_grafico():
      ...
      ultimas_cotacoes=rq.get('https://economia.awesomeapi.com.br/json/daily/BTC-BRL/30')
      ...
      plt.grid(True)
      #Titulo
      plt.title('Últimas 30 Cotações do Bitcoin (BTC/BRL)')
      #Nome eixo x
      plt.xlabel('Data')
      #Nome eixo y
