...
from cotacao import cotacao,cotacao_dollar
...
def dollar():
   #Criando a janela 
    ...
    #Texto 
    text=ctk.CTkLabel(window,text='DOLLAR',font=fonte)
    #Coloca o texto na tela
    text.grid(padx=120,pady=15)

    #Texto 
    text=ctk.CTkLabel(window,text=f'COTAÇÃO: R$ {float(cotacao_dollar()):.3f}',font=fonte)
    #Coloca o texto na tela
    text.grid()
    ...
    def converter():
        ...
        qnt_dollar=conversaoInversa.get()
        resultado_texto=""

        try:
          if qnt_real :
            qnt_real=float(qnt_real)
            montante_dollar=qnt_real/cotacao_dollar()
            resultado_texto=f'R$ {qnt_real:.2f} -> {montante_dollar:.2f} $'
          elif qnt_dollar:
            qnt_dollar=float(qnt_dollar)
            montante_real=cotacao_dollar()*qnt_dollar
            resultado_texto=f'{qnt_dollar:.2f} $ -> R$ {montante_real:.2f}'
          else:
    ...
    def mostrar_grafico():
      ...
      ultimas_cotacoes=rq.get('https://economia.awesomeapi.com.br/json/daily/USD-BRL/30')
      ...
      plt.grid(True)
      #Titulo
      plt.title('Últimas 30 Cotações do Dollar (USD/BRL)')
      #Nome eixo x
      plt.xlabel('Data')
      #Nome eixo y
