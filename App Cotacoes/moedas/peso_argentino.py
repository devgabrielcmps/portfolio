...
from cotacao import cotacao,cotacao_peso_argentino
...
def peso_argentino():
    ...
    #Texto 
    text=ctk.CTkLabel(window,text='PESO ARGENTINO',font=fonte)
    #Coloca o texto na tela
    text.grid(padx=70,pady=15)

    #Texto 
    text=ctk.CTkLabel(window,text=f'COTAÇÃO: R$ {float(cotacao_peso_argentino()):.3f}',font=fonte)
    #Coloca o texto na tela
    text.grid()
    ...
    def converter():
        ...
        qnt_peso=conversaoInversa.get()
        resultado_texto=""

        try:
          if qnt_real :
            qnt_real=float(qnt_real)
            montante_peso=qnt_real/cotacao_peso_argentino()
            resultado_texto=f'R$ {qnt_real:.2f} -> {montante_peso:.2f} $'
          elif qnt_peso:
            qnt_peso=float(qnt_peso)
            montante_real=cotacao_peso_argentino()*qnt_peso
            resultado_texto=f'{qnt_peso:.2f} $ -> R$ {montante_real:.2f}'
          else:
    ...
    def mostrar_grafico():
      ...
      ultimas_cotacoes=rq.get('https://economia.awesomeapi.com.br/json/daily/ARS-BRL/30')
      ...
      plt.grid(True)
      #Titulo
      plt.title('Últimas 30 Cotações do Peso Argentino (ARS/BRL)')
      #Nome eixo x
      plt.xlabel('Data')
      #Nome eixo y
